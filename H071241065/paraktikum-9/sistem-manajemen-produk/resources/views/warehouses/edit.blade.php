  ...
  <form action="{{ route('warehouses.update', $warehouse->id) }}" method="POST">
    ...
    <div class="mb-4">
      ...
      <textarea id="location" name="location" rows="4"
        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('location') border-red-500 @enderror">{{ old('location', $warehouse->location) }}</textarea>
      @error('location')
        <p class="text-red-500 text-xs italic">{{ $message }}</p>
      @enderror
    </div>
